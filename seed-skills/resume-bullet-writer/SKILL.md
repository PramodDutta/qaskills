...
license: MIT
tags: [resume, writing, career, achievements]
testingTypes: [career]
frameworks: []
languages: [markdown]
domains: [web]
...
QA bullets sink into task language faster than any other role: "wrote test cases", "performed regression testing", "logged defects". Convert each to scope + method + outcome: "Built a 400-case Playwright regression suite that cut release sign-off from 3 days to 4 hours" or "Caught 12 critical defects pre-release by targeting boundary and negative paths in the payments flow". Countable QA outcomes: escaped-defect rate, automation coverage, suite runtime, flake rate, release cadence, production incidents prevented.
