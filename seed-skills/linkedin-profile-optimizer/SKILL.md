...
license: MIT
tags: [linkedin, career, personal-brand, job-search]
testingTypes: [career]
frameworks: []
languages: [markdown]
domains: [web]
...
Recruiters search LinkedIn for QA roles with tool + title strings: "SDET Playwright", "QA automation Selenium Java", "API testing engineer". Your headline must carry the searchable tokens, not a slogan: "SDET | Playwright, TypeScript, CI/CD | API + E2E Automation" outranks "Quality enthusiast breaking software daily". Fill the Skills section with the exact frameworks list and pin the top three. Posts that show real test code or a flake-hunting war story convert profile views into recruiter messages far better than motivational reposts.
