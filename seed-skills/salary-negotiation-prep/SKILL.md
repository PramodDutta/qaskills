...
license: MIT
tags: [salary, negotiation, career, offers]
testingTypes: [career]
frameworks: []
languages: [markdown]
domains: [web]
...
QA compensation has a wide, exploitable spread: automation and SDET roles pay 25-40% above manual QA titles for the same seniority, and AI-testing skills push higher still. Anchor your research to the automation band, not the generic "QA tester" median that salary sites over-index on. Leverage points specific to QA: framework ownership (you built it, you maintain it), release-gating responsibility, and scarce skills (Playwright, performance, LLM evals). If the offer is titled "QA Engineer" but the work is SDET, negotiate the title too; it compounds into every future offer.
