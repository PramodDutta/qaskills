...
license: MIT
tags: [portfolio, resume, career, design]
testingTypes: [career]
frameworks: []
languages: [markdown]
domains: [web]
...
Visual resume formats are risky for QA applications (ATS parsing, conservative screeners), so keep the applying resume plain and put the creativity where it converts: the portfolio site. A QA portfolio that stands out has a live demo suite visitors can run, a bug-hunt writeup with screenshots, dashboards from your reporting stack, and a short "how I test" page. Link it from the one-column resume. If you do submit a designed resume anywhere, submit the plain version to the portal and hand the designed one to humans.
