...
Some iteration discipline that pays off: keep the dataset under version control so eval results are reproducible; add a new test case every time you find a real-world failure so regressions cannot sneak back; and watch the partial-credit scores, not just pass/fail, because a drop from 1.0 to 0.5 on the trace grader means your agent started doing extra work even if it still technically passes. When pass rates plateau, expand the dataset with harder edge cases rather than declaring victory. For adversarial test cases that probe agent safety, see the [promptfoo red teaming guide](/blog/promptfoo-red-teaming-llm-applications), and for guardrail testing patterns see the [LLM guardrails testing guide](/blog/llm-guardrails-testing-guide-2026).
