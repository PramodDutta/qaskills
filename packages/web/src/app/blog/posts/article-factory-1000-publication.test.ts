...
import { articleFactory1000Posts } from './_article-factory-1000-2026-07-26';
...
const selectedReport = JSON.parse(
  fs.readFileSync(
    path.resolve(REPO_ROOT, 'docs/seo/article-factory-1000-2026-07-26/selected-campaign.json'),
    'utf8',
  ),
) as {
  selected: Array<{
...
const baselineInventory = JSON.parse(
  fs.readFileSync(
    path.resolve(REPO_ROOT, 'docs/seo/article-factory-250-2026-07-25/inventory-baseline.json'),
    'utf8',
  ),
) as {
  items: Array<{
...
const campaignSlugs = new Set(articleFactory1000Posts.map(({ slug }) => slug));
const extensionSlugs = new Set(articleFactory1000ExtensionPosts.map(({ slug }) => slug));
const legacyArticles = baselineInventory.items.filter(({ kind }) => kind === 'article');
const bannedPhrases = [
  'delve',
...
describe('1,000-article codebase-driven SEO factory', () => {
  it('registers the 250-post checkpoint and 750-post extension exactly once', () => {
    expect(articleFactory1000Posts).toHaveLength(1_000);
    expect(articleFactory1000ExtensionPosts).toHaveLength(750);
    expect(campaignSlugs.size).toBe(1_000);
    expect(extensionSlugs.size).toBe(750);
    expect(new Set(articleFactory1000Posts.map(({ post }) => post.title)).size).toBe(1_000);
    expect(new Set(articleFactory1000Posts.map(({ post }) => post.primaryKeyword)).size).toBe(
      1_000,
    );
    expect(candidatesBySlug.size).toBe(1_000);

    for (const { slug, post } of articleFactory1000Posts) {
  ...
  it('meets campaign metadata, evidence, and source contracts', () => {
    for (const { slug, post } of articleFactory1000Posts) {
      ...
      expect(['2026-07-25', '2026-07-26'], `${slug} date`).toContain(post.date);
      expect(['2026-07-25', '2026-07-26'], `${slug} updated`).toContain(post.updated);
  ...
  it('meets length, readability, structure, FAQ, and linking thresholds', () => {
    for (const { slug, post } of articleFactory1000Posts) {
      ...
      const headings = Array.from(contentWithoutCode.matchAll(/^##(?!#)\s+(.+)$/gm), (match) =>
        match[1].trim(),
      );
      ...
      for (const questionHeading of headings.filter((heading) => heading.endsWith('?'))) {
        const start = contentWithoutCode.indexOf(`## ${questionHeading}`);
        const answer = contentWithoutCode
  ...
  it('contains no banned text and does not collide with the pre-factory inventory', () => {
    for (const { slug, post } of articleFactory1000Posts) {
      ...
      for (const legacy of legacyArticles) {
        ...
        expect(
          tokenOverlap(post.title, legacy.title || legacy.h1),
