...
Here the app reads the \`msw-enabled\` flag on boot and starts \`setupWorker\` before rendering. For a full treatment of browser-level interception in Playwright itself, read [Playwright network mocking](/blog/playwright-network-mocking-route-handler-guide).
...
Want more testing recipes? Explore the [QA skills directory](/skills), compare runners in [Vitest vs Jest in 2026](/blog/vitest-vs-jest-2026-comparison), and pair MSW with browser-level interception via [Playwright network mocking](/blog/playwright-network-mocking-route-handler-guide).
`,
};
