...
\`\`\`bash
# Start Burp Suite, then configure your browser proxy to 127.0.0.1:8080.
# For mobile testing, install the Burp CA certificate on the test device.
# Keep interception off during normal browsing, turn it on only when capturing a specific request.
\`\`\`
...
\`\`\`js
const target = 'https://staging.example.com/api/orders/123';
const headers = {
  Authorization: 'Bearer replace-with-test-token',
...
**proxy history** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**repeater** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**intruder** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**decoder** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**sequencer** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

These concepts should appear in your test plan. A test plan that says only run Burp Suite is too vague. A useful plan names the targets, data state, expected thresholds, severity rules, known exceptions, and artifact location. It also states what will not be covered. Exclusions are healthy when they are explicit. They are dangerous when they are accidental.

| Area | What QA should verify | Evidence to keep | Release decision |
|---|---|---|---|
| Setup | Burp Suite runs with pinned configuration and known targets | Command, config file, environment name | Do not gate until setup is repeatable |
...
Start by writing down the user promise in plain English. A customer can sign in, review a cart or order, complete the action, and receive a clear result without losing data or crossing authorization boundaries. For Burp Suite, translate that promise into three layers. The first layer is a smoke check that proves the target is reachable and in the expected state. The second layer runs the workflow with stable inputs. The third layer stores evidence so a failure can be debugged after the CI job expires.
...
Best practice starts with ownership. A Burp Suite failure should have a clear route to the right team: frontend, backend, platform, security, design, content, or product. When ownership is vague, findings sit in dashboards until they become background noise. Put ownership in the defect template and in the rule documentation.
...
- Gate on reproducible authorization bypass only after the team has a reproduction path and an agreed owner.
- Gate on sensitive data exposure only after the team has a reproduction path and an agreed owner.
- Gate on missing CSRF protection only after the team has a reproduction path and an agreed owner.
- Gate on weak session behavior only after the team has a reproduction path and an agreed owner.
...
Common mistakes show up early with Burp Suite. The first is treating a default scan as a policy. Defaults are a starting point, not your release standard. A rule or metric only deserves to block a release when the team understands the failure mode, can reproduce it, and agrees on the fix path.
...
The fourth mistake is ignoring test data. Burp Suite can only evaluate the state it sees. If staging data is empty, permissions are unrealistic, feature flags differ from production, or third party scripts are disabled in CI, the result can be technically correct and still useless. Stable fixtures are not busywork. They are the foundation of trustworthy automated quality signals.

## Comparison With Alternatives

No single tool owns security quality. Burp Suite should be selected because it fits the workflow, not because it is fashionable. The comparison below is deliberately practical. It focuses on how a QA team will use the option, where it is strong, and where it needs help.

| Option | Strength | Weakness | Best fit |
|---|---|---|---|
| OWASP ZAP | Good complementary coverage for another layer of risk | May require extra setup or paid access | Regression checks on critical flows |
| Nuclei | Often easier for a specialist reviewer to interpret | Can be noisy without scoped targets | Manual investigation before release |
| Postman negative tests | Can catch issues outside the default automated path | Needs human judgment for prioritization | Periodic audit or compliance evidence |
| commercial DAST scanners | Useful when the team needs an independent second signal | Does not replace product-specific acceptance criteria | Cross-checking a risky change |

The right answer may be a combination. For example, a team might use Burp Suite for pull request checks, a second tool for periodic discovery, and manual review for high risk releases. That is not duplication if each layer answers a different question. It is duplication only when two tools create the same noisy finding and nobody owns the result.

## CI Integration

CI integration should be boring on purpose. Put Burp Suite behind a named script, run it after the application is built or deployed to a stable preview, upload the report as an artifact, and fail only on rules that the team has agreed to own. If the job needs credentials, use least privilege test accounts and short lived secrets where your platform supports them.

For pull requests, use a narrow gate. For nightly or scheduled jobs, run broader discovery. For release branches, run the gate against the same artifact or environment that will be promoted. This split keeps developer feedback fast while still giving QA enough coverage before production. Teams using GitHub Actions can map this pattern to status checks, artifacts, environment approvals, and scheduled workflows; the companion [DAST versus SAST and SCA guide](/blog/dast-vs-sast-vs-sca-qa-guide-2026) is the right next stop when you need the pipeline details.
...
\`\`\`yaml
name: burp-supported-security-checks
on: [pull_request]
jobs:
  api-negative-tests:
...
### How does Burp Suite fit with manual QA?
