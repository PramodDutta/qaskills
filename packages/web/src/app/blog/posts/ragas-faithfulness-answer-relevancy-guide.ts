...
Set thresholds from a baseline run, not from wishful thinking: run the eval on your current main branch, see where you actually land, and set floors slightly below that so genuine regressions trip the gate while normal noise does not. For more on gating patterns across frameworks, see the [DeepEval testing framework guide](/blog/deepeval-llm-testing-guide) and the [OpenAI Evals complete guide](/blog/openai-evals-complete-guide-2026).
