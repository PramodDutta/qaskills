...
export const post: BlogPost = {
  title: 'Lighthouse CI Accessibility Testing: Budgets, Scores, and GitHub Actions',
  description: 'Lighthouse CI accessibility testing guide for QA teams: set budgets, interpret scores, add GitHub Actions gates, and prevent regressions before release.',
  date: '2026-07-09',
  category: 'Tutorial',
  content: `
...
**Lighthouse categories** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**audit assertions** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**minimum scores** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**page sampling** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**report upload storage** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

These concepts should appear in your test plan. A test plan that says only run Lighthouse CI is too vague. A useful plan names the targets, data state, expected thresholds, severity rules, known exceptions, and artifact location. It also states what will not be covered. Exclusions are healthy when they are explicit. They are dangerous when they are accidental.

| Area | What QA should verify | Evidence to keep | Release decision |
|---|---|---|---|
| Setup | Lighthouse CI runs with pinned configuration and known targets | Command, config file, environment name | Do not gate until setup is repeatable |
...
- Gate on accessibility score below threshold only after the team has a reproduction path and an agreed owner.
- Gate on critical audit failure only after the team has a reproduction path and an agreed owner.
- Gate on new contrast failure only after the team has a reproduction path and an agreed owner.
...
Common mistakes show up early with Lighthouse CI. The first is treating a default scan as a policy. Defaults are a starting point, not your release standard. A rule or metric only deserves to block a release when the team understands the failure mode, can reproduce it, and agrees on the fix path.
...
The fourth mistake is ignoring test data. Lighthouse CI can only evaluate the state it sees. If staging data is empty, permissions are unrealistic, feature flags differ from production, or third party scripts are disabled in CI, the result can be technically correct and still useless. Stable fixtures are not busywork. They are the foundation of trustworthy automated quality signals.

## Comparison With Alternatives

No single tool owns accessibility quality. Lighthouse CI should be selected because it fits the workflow, not because it is fashionable. The comparison below is deliberately practical. It focuses on how a QA team will use the option, where it is strong, and where it needs help.

| Option | Strength | Weakness | Best fit |
|---|---|---|---|
| axe-core CLI | Good complementary coverage for another layer of risk | May require extra setup or paid access | Regression checks on critical flows |
| Playwright accessibility snapshots | Often easier for a specialist reviewer to interpret | Can be noisy without scoped targets | Manual investigation before release |
| Pa11y CI | Can catch issues outside the default automated path | Needs human judgment for prioritization | Periodic audit or compliance evidence |
| manual screen reader testing | Useful when the team needs an independent second signal | Does not replace product-specific acceptance criteria | Cross-checking a risky change |

The right answer may be a combination. For example, a team might use Lighthouse CI for pull request checks, a second tool for periodic discovery, and manual review for high risk releases. That is not duplication if each layer answers a different question. It is duplication only when two tools create the same noisy finding and nobody owns the result.

## CI Integration

CI integration should be boring on purpose. Put Lighthouse CI behind a named script, run it after the application is built or deployed to a stable preview, upload the report as an artifact, and fail only on rules that the team has agreed to own. If the job needs credentials, use least privilege test accounts and short lived secrets where your platform supports them.
...
### Should Lighthouse CI block every pull request?
...
### How does Lighthouse CI fit with manual QA?
