...
This table is also a guardrail. If a team expects Factory Boy to replace exploratory testing, it will fail. If the team expects it to catch a defined class of boundary regression quickly, it can become one of the highest-return checks in the pipeline.
...
\`\`\`bash
pip install factory_boy pytest Faker
pytest tests/test_factories.py
\`\`\`

Keep the first configuration small. A minimal but trusted check is better than a large rule set that produces noise. Commit the artifact near the code that owns it. If the artifact is generated, regenerate it in CI before running Factory Boy. If it is manually maintained, lint it and review it like production code.
...
The first concept is source of truth. A source of truth is the artifact that consumers and tests should trust. In this workflow the source of truth is factories.py. If generated clients, mocks, docs, or tests disagree with that file, decide which artifact wins and automate the rest from it when possible.
...
\`\`\`python
def test_paid_order_is_returned_by_api(client):
    order = OrderFactory(status='paid')
    client.post('/test-support/orders', json=order.to_dict())
...
CI turns Factory Boy from a helpful local command into a quality gate. The job should run when factories.py or related source files change. It should publish logs or reports as artifacts. It should fail only when the team agrees the finding is release-blocking.

\`\`\`yaml
factory_boy_test_data_guide_2026:
  image: node:20
  script:
    - npm ci
    - npm run lint
...
| Keep artifacts close | factories.py is versioned with owners and review rules | The rule lives in a wiki or private script |
...
The first mistake is using Factory Boy as a substitute for design review. A green check does not prove the change is desirable. It proves that a known rule still holds. Use the signal to improve review, not to remove judgment.
...
Factory Boy is a good fit for Python object factory workflow, but it is not the only option. Consider alternatives when the risk is outside the tool's natural scope.

- static fixtures for golden scenarios
- factory libraries for relational object graphs
- masked production snapshots when high-fidelity data is required
...
Keep secrets in the CI secret store, not in factories.py. Mask logs. Avoid committing production payloads as examples. Prefer synthetic data unless a masked production sample is truly required. When a check stores artifacts, review retention and access. A helpful test report should not become a data leak.
...
1. factories.py is versioned and has an owner.
...
### Who should own the factories.py workflow?
