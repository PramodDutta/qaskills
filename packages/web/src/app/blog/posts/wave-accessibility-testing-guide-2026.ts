...
# 2. Open the target page in Chrome, Edge, or Firefox.
# 3. Run WAVE and export or screenshot the panel for evidence.

# API style smoke check when your team has API access
curl -s "https://wave.webaim.org/api/request?key=$WAVE_API_KEY&url=https://example.com"
\`\`\`
...
test('accessibility triage page has stable landmarks before WAVE review', async ({ page }) => {
...
**errors** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**alerts** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**features** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**structure** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

**contrast checks** is part of the working vocabulary for this topic. Define it in your team docs, show where it appears in reports, and connect it to a decision the QA team actually makes.

These concepts should appear in your test plan. A test plan that says only run WAVE is too vague. A useful plan names the targets, data state, expected thresholds, severity rules, known exceptions, and artifact location. It also states what will not be covered. Exclusions are healthy when they are explicit. They are dangerous when they are accidental.

| Area | What QA should verify | Evidence to keep | Release decision |
|---|---|---|---|
| Setup | WAVE runs with pinned configuration and known targets | Command, config file, environment name | Do not gate until setup is repeatable |
...
Best practice starts with ownership. A WAVE failure should have a clear route to the right team: frontend, backend, platform, security, design, content, or product. When ownership is vague, findings sit in dashboards until they become background noise. Put ownership in the defect template and in the rule documentation.
...
- Gate on verified WAVE error only after the team has a reproduction path and an agreed owner.
- Gate on unresolved contrast issue only after the team has a reproduction path and an agreed owner.
- Gate on missing form label only after the team has a reproduction path and an agreed owner.
- Gate on incorrect heading structure only after the team has a reproduction path and an agreed owner.
...
Common mistakes show up early with WAVE. The first is treating a default scan as a policy. Defaults are a starting point, not your release standard. A rule or metric only deserves to block a release when the team understands the failure mode, can reproduce it, and agrees on the fix path.
...
The fourth mistake is ignoring test data. WAVE can only evaluate the state it sees. If staging data is empty, permissions are unrealistic, feature flags differ from production, or third party scripts are disabled in CI, the result can be technically correct and still useless. Stable fixtures are not busywork. They are the foundation of trustworthy automated quality signals.

## Comparison With Alternatives

No single tool owns accessibility quality. WAVE should be selected because it fits the workflow, not because it is fashionable. The comparison below is deliberately practical. It focuses on how a QA team will use the option, where it is strong, and where it needs help.

| Option | Strength | Weakness | Best fit |
|---|---|---|---|
| axe DevTools | Good complementary coverage for another layer of risk | May require extra setup or paid access | Regression checks on critical flows |
| Lighthouse | Often easier for a specialist reviewer to interpret | Can be noisy without scoped targets | Manual investigation before release |
| Pa11y | Can catch issues outside the default automated path | Needs human judgment for prioritization | Periodic audit or compliance evidence |
| manual assistive technology review | Useful when the team needs an independent second signal | Does not replace product-specific acceptance criteria | Cross-checking a risky change |

The right answer may be a combination. For example, a team might use WAVE for pull request checks, a second tool for periodic discovery, and manual review for high risk releases. That is not duplication if each layer answers a different question. It is duplication only when two tools create the same noisy finding and nobody owns the result.

## CI Integration

CI integration should be boring on purpose. Put WAVE behind a named script, run it after the application is built or deployed to a stable preview, upload the report as an artifact, and fail only on rules that the team has agreed to own. If the job needs credentials, use least privilege test accounts and short lived secrets where your platform supports them.

For pull requests, use a narrow gate. For nightly or scheduled jobs, run broader discovery. For release branches, run the gate against the same artifact or environment that will be promoted. This split keeps developer feedback fast while still giving QA enough coverage before production. Teams using GitHub Actions can map this pattern to status checks, artifacts, environment approvals, and scheduled workflows; the companion [web testing checklist](/blog/web-testing-checklist-2026) is the right next stop when you need the pipeline details.
...
\`\`\`yaml
name: wave-assisted-accessibility
on: [pull_request]
jobs:
  accessibility-smoke:
...
### How does WAVE fit with manual QA?
